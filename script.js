...
function initSLTPCalculator() {
    ...
    // Helper function to detect pip size based on entry price
    function detectPipSize(price) {
        // For forex pairs like EURUSD (price around 1.xxxx)
        if (price < 10) {
            // Check if 5 decimal places (e.g., 1.12345)
            if (price.toString().includes('.') && price.toString().split('.')[1].length >= 5) {
                return 0.00001; // 5 decimal forex (0.1 pip = 0.00001)
            } else {
                return 0.0001; // 4 decimal forex (0.1 pip = 0.0001)
            }
        } 
        // For JPY pairs (price around 100-150)
        else if (price >= 10 && price < 1000) {
            return 0.01; // JPY pairs (0.1 pip = 0.01)
        } 
        // For indices, stocks, etc.
        else {
            return 0.1; // Indices, stocks, etc. (0.1 pip = 0.1)
        }
    }
...
function initRRRWinRateCalculator() {
    ...
    calculateButton.addEventListener('click', function() {
        ...
        // ก่อนอื่นคำนวณ winsNeeded จาก floor(minWinRate)
        const baseWins = Math.round(Math.floor(minWinRate) * tradesPerMonth / 100);

        // เริ่มต้นที่ ceil(minWinRate)
        let ceilMinWinRate = Math.ceil(minWinRate);

        // เพิ่มจนกว่า winsNeeded ที่คำนวณจาก ceilMinWinRate จะมากกว่า baseWins (เพิ่มขึ้น 1 ครั้ง)
        while (Math.floor(ceilMinWinRate * tradesPerMonth / 100) <= baseWins) {
            ceilMinWinRate++;
        }

        const winRates = [
            Math.max(5, Math.floor(minWinRate) - 10), 
            Math.floor(minWinRate),
            ceilMinWinRate,
            Math.min(80, ceilMinWinRate + 10),
            Math.min(90, ceilMinWinRate + 20)
        ];
        
        // Add rows to win rate table
        winRates.forEach(winRate => {
            ...
            const winsNeeded = Math.round(winRate * tradesPerMonth / 100);
